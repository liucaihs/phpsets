            ...
            <div class="panel panel-info">
                ...
                <form class="form-inline" role="form" style="margin: 10px;" method="get">
                    ...
                    <div class="form-group">
                        <select class="form-control" id="adput_name" name="adput_name" onchange="onclis()">
                        </select>
                    </div>
                    <label>结算方式</label>
                    <div class="form-group">
...
function getall_corporation(){
    $.ajax({
        ...
        success: function(data){
            // console.log(data);
            var querySelect = document.getElementById("adput_name");
            // var oOp = querySelect.children;//获取select列表的所有子元素。
            if(data){
                querySelect.options.add(new Option(" ==选择广告== ",""));
                for(var i in data){
                    querySelect.options.add(new Option(data[i].adput_name, data[i].adput_name));
                }
            }
        },
        beforeSend: function(){
