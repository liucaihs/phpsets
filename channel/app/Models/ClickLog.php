...
class ClickLog extends CommModel
{
    ...
    protected function apendData(&$data)
    {
        ...
        foreach ($data as $row) {
            ...
            $active_nums = 0;//Activated::where('adput_sn', $row->adput_sn)->where(DB::raw("FROM_UNIXTIME(create_time, '%Y-%m-%d')") ,$row->days)->count(DB::raw("distinct raw_data"));
            ...
            $row->setAttribute('into_price', $ad_puton->price);
    ...
    public function reckonTotal($data)
    {
        ...
        $totalData = ['show_nums' => 0 , 'click_nums' => 0 , 'active_nums' => 0 , 'notclasp_nums' => 0 , 'costprice' => 0 , 'active_percent' => '0%'];
        foreach ($reset as $row) {
            $totalData['click_nums'] += $row->click_nums;
            $totalData['active_nums'] += $row->active_nums;
            $totalData['notclasp_nums'] += $row->notclasp_nums;
            $totalData['costprice'] += $row->costprice;
        }
        if ($totalData['click_nums'] > 0) {
            $totalData['active_percent'] = round($totalData['notclasp_nums']/$totalData['click_nums'] * 100, 2) . '%';
        }
        return $totalData;
    }
}
